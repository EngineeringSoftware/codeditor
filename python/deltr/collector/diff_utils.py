...
def compute_code_diffs(old_tokens: List[str], new_tokens: List[str]):
    spans = []
    tokens = []
    commands = []
...
def compute_diffs_no_keep(old_tokens: List[str], new_tokens: List[str]):
    spans = []
    del_tokens = []
    add_tokens = []
    tokens = []
    ...
    for edit_type, o_start, o_end, n_start, n_end in difflib.SequenceMatcher(
        ...
        if edit_type == "replace":
            ...
            for i in range(o_start, o_end):
                tokens.extend([REPLACE_OLD, old_tokens[i]])
                commands.append(REPLACE_OLD)
                del_tokens.extend([old_tokens[i]])
            for j in range(n_start, n_end):
                tokens.extend([REPLACE_NEW, new_tokens[j]])
                commands.extend([REPLACE_NEW, new_tokens[j]])
                add_tokens.extend([new_tokens[j]])
        elif edit_type == "insert":
            spans.extend([INSERT] + new_tokens[n_start:n_end] + [INSERT_END])
            for j in range(n_start, n_end):
                tokens.extend([INSERT, new_tokens[j]])
    ...
    return spans, tokens, add_tokens, del_tokens


def compute_unique_edits(old_tokens, new_tokens):
    """Compute the unique edits w.o. keep sequence."""
    spans = []
    unique_spans = []
    ...
    for edit_type, o_start, o_end, n_start, n_end in difflib.SequenceMatcher(
        ...
        if edit_type == "equal":
            ...
            spans.extend([INSERT] + new_tokens[n_start:n_end] + [INSERT_END])
            unique_spans.append([INSERT] + new_tokens[n_start:n_end] + [INSERT_END])
        else:
            spans.extend([DELETE] + old_tokens[o_start:o_end] + [DELETE_END])
            unique_spans.append([DELETE] + old_tokens[o_start:o_end] + [DELETE_END])
    edit_plan = []
    for span in unique_spans:
        if span not in edit_plan:
            edit_plan.append(span)
    spans = [" ".join(span) for span in edit_plan]
    edit_plan = " ".join([token for span in edit_plan for token in span])
    return spans, edit_plan
...
def compute_minimal_comment_diffs(
        ...
        if node.edit_type == KEEP:
            ...
            if node.next and node.next.edit_type == KEEP:
                ...
                if found_substring:
                    ...
                    if node.next.next.edit_type == DELETE:
                        node.next.next.edit_type = REPLACE
                        node.next.next.children = (
                            node.children
                            + adopted_children
                            + node.next.next.children
                            + [REPLACE_NEW]
                        ...
                        node.next.edit_type = None
                        continue
                    elif node.next.next.edit_type == REPLACE:
                        rep_idx = node.next.next.children.index(REPLACE_NEW)
                        next_replace_rep_old_children = node.next.next.children[
                            :rep_idx
                        ]
                        next_replace_rep_new_children = node.next.next.children[
                            rep_idx + 1 :
                        ]
                        ...
                        node.next.next.children = (
                            node.children
                            + adopted_children
                            + next_replace_rep_old_children
                            + [REPLACE_NEW]
                            + adopted_children
                            + next_replace_rep_new_children
                        )
                        # re-link node
                        if node.prev:
                            node.prev.next = node.next.next
                        ...
                        node.next.edit_type = None
                        continue
                    elif node.next.next.edit_type == INSERT:
                        node.next.next.children = (
                            node.children
                            + adopted_children
                            + [REPLACE_NEW]
                        ...
                        raise RuntimeError("Un-parsable seq.")
            ...
            if node.next and node.next.edit_type == KEEP:
                ...
                if found_substring:
                    ...
                    if node.next.next.edit_type == DELETE:
                        ...
                        node.next.next.edit_type = REPLACE
                        node.next.next.children = new_children
                        # re-link
                        if node.prev:
                            node.prev.next = node.next.next
                        node.next.next.prev = node.prev
                        ...
                        rep_idx = node.next.next.children.index(REPLACE_NEW)
                        next_rep_old_children = node.next.next.children[:rep_idx]
                        next_rep_new_children = node.next.next.children[rep_idx + 1 :]
                        new_children = (
                            rep_old_children
                            + adopted_children
                            + next_rep_old_children
                            + [REPLACE_NEW]
                            + rep_new_children
                            + adopted_children
                            + next_rep_new_children
                        )
                        node.next.next.children = new_children
                        # re-link
                        if node.prev:
...
def compute_minimal_replace_diffs(
    old_tokens, new_tokens
) -> Tuple[List[str], List[str], List[str]]:
    spans = []
    tokens = []
...
def is_delete(token):
    return "DELETE" in token and "OLD" not in token and "NEW" not in token
...
def is_incomplete_edits_spans(diff_span_tokens: List[str]):
    """
    Check if the generated edit span is complete.
    e.g. NEW should accompany with END
    """
    ...
    while i < len(diff_span_tokens):
        ...
        if is_delete(token):
            ...
            while j < len(diff_span_tokens) and not is_delete_end(diff_span_tokens[j]):
                j += 1
            if j == len(diff_span_tokens) and not is_delete_end(
                diff_span_tokens[j - 1]
            ):
            ...
            while j < len(diff_span_tokens) and not is_insert_new(diff_span_tokens[j]):
                j += 1
            if j == len(diff_span_tokens) and not is_insert_new(
                diff_span_tokens[j - 1]
            ):
            ...
            while j < len(diff_span_tokens) and not is_insert_end(diff_span_tokens[j]):
                j += 1
            if j == len(diff_span_tokens) and not is_insert_end(
                diff_span_tokens[j - 1]
            ):
                return "InsertNotComplete"
            ...
            while j < len(diff_span_tokens) and not is_replace_new(diff_span_tokens[j]):
                j += 1
            if j == len(diff_span_tokens) and not is_replace_new(
                diff_span_tokens[j - 1]
            ):
                return "ReplaceNotComplete"
            j += 1
            while j < len(diff_span_tokens) and not is_replace_end(diff_span_tokens[j]):
                j += 1
            if j == len(diff_span_tokens) and not is_replace_end(
                diff_span_tokens[j - 1]
            ):
                return "ReplaceNotComplete"
        else:
...
def format_minimal_diff_spans(reference_tokens: List[str], diff_span_tokens: List[str]):
    ...
    while i < len(diff_span_tokens):
        ...
        if is_delete(token):
            ...
            )  # idx: the index of matched spans, d_start: the index of matched spans in the whole delete span
            ...
            if idx >= 0:
                ...
                # ref: a b c d  span: <delete> c b c d <delete_end> --> a c
                ...
                ptr += idx
                # ref: a b c d  span: <delete> b c e <delete_end> --> a e
...
def remove_keep_span(edit_seq: Union[str, List[str]]) -> str:
    span_no_keep = []
    if type(edit_seq) == str:
        tokenized_edits = edit_seq.split()
    else:
        tokenized_edits = edit_seq
    i = 0
    while i < len(tokenized_edits):
        ...
        if tk == "<KEEP>":
            while tokenized_edits[i] != "<KEEP_END>" and i + 1 < len(tokenized_edits):
                i = i + 1
        elif tk == "<KEEP_END>":
            i += 1
            continue
...
if __name__ == "__main__":
    # test cases
    old_code = "<replace_old_keep_before> resultRenderer ) { <replace_new_keep_before> resultRenderer ) { Transform transformProp = resultRenderer . < Transform > getProperty ( Property . TRANSFORM ) ; Border outlineProp = resultRenderer . < Border > getProperty ( Property . OUTLINE ) ; <replace_end> <replace_old_keep_before> resultRenderer ) && <replace_new_keep_before> resultRenderer ) ) { processWaitingDrawing ( resultRenderer , transformProp , outlineProp , waitingDrawingElements ) ; if <replace_end> <replace_old> resultRenderer . < Transform > getProperty ( Property . TRANSFORM ) != null ) ) { waitingDrawingElements . add ( resultRenderer ) ; <replace_new> transformProp != null ) <replace_end> <eos>"
    new_code = "<replace_old> if ( ! waitingDrawingElements . Contains ( resultRenderer ) && ( FloatingHelper . IsRendererFloating ( resultRenderer ) || <replace_new> Transform transformProp = <replace_end> <replace_old> != null ) ) { <replace_new> ; Border outlineProp = resultRenderer . GetProperty < Border > ( Property . OUTLINE ) ; if ( ! <replace_end> <replace_old> Add ( resultRenderer ) ; <replace_new> Contains ( resultRenderer ) ) { ProcessWaitingDrawing ( resultRenderer , transformProp , outlineProp , waitingDrawingElements ) ; if ( FloatingHelper . IsRendererFloating ( resultRenderer ) || transformProp != null ) { <replace_end> <replace_old_keep_before> return ; <replace_new_keep_before> return ; } <replace_end> <eos>"

    edits = compute_minimal_replace_diffs(old_code.split(), new_code.split())[0]
    print(" ".join(edits))
    # final_pred = format_minimal_diff_spans(old_code.split(), edits)
    # print(final_pred == new_code)
    # print(final_pred)
